...
type routeTestRepo struct {
	...
	routeTokens map[string]storedHTTPRouteToken
}

type storedHTTPRouteToken struct {
...
func (f *routeTestRepo) CreateRouteToken(_ context.Context, token routes.RouteToken, digest []byte) error {
	stored := token
	stored.Secret = ""
	f.routeTokens[token.ID] = storedHTTPRouteToken{token: stored, digest: append([]byte(nil), digest...)}
	return nil
}

func (f *routeTestRepo) ListRouteTokens(_ context.Context, owner domain.OwnerID, routeID domain.RouteID) ([]routes.RouteToken, error) {
...
func (f *routeTestRepo) RevokeRouteToken(_ context.Context, owner domain.OwnerID, routeID domain.RouteID, id string, at time.Time) error {
...
func (f *routeTestRepo) FindRouteToken(_ context.Context, digest []byte, _ time.Time) (routes.RouteToken, error) {
	for _, item := range f.routeTokens {
		if item.revoked != nil {
			continue
		}
